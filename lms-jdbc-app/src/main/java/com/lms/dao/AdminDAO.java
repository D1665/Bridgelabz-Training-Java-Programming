    ...
    private void seedDefaultAdmin() {
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            try (PreparedStatement check = conn.prepareStatement(checkSql)) {
                check.setString(1, "dineshsirsiya@gmail.com");
                try (ResultSet rs = check.executeQuery()) {
                    if (rs.next() && rs.getInt(1) == 0) {
                        try (PreparedStatement insert = conn.prepareStatement(insertSql)) {
                            insert.setString(1, "User Admin");
                            insert.setString(2, "dineshsirsiya@gmail.com");
                            insert.setString(3, "sirsiya@12");
                            insert.executeUpdate();
                            System.out.println("Default admin account seeded (dineshsirsiya@gmail.com).");
                        }
                    }
                }
            }
